...
def without_duplicates(words):
    ...
    words = set(words) #using the set data-type to remove duplicates
    return list(words) #changing set to a list
    ...
    for i in range(len(numbers)):
        for j in range(1,len(numbers)):
            if numbers[i] + numbers[j] == 0:
                sets.append(list((numbers[j], numbers[i])))
            else:
                pass
    return sets[:len(numbers)//2]
